...
import { buildConversationContext, getMessageAuthor } from './conversation-context';
...
export class TelegramService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit(): Promise<void> {
    ...
    this.botUsername = this.configService.get<string>('BOT_USERNAME') || botInfo.username || '';

    await this.bot.launch({
      dropPendingUpdates: this.getBoolean('BOT_DROP_PENDING_UPDATES', true)
    });

    this.logger.log(`Telegram bot started as @${this.botUsername || botInfo.username}`);
  }
  ...
  private registerHandlers(bot: Telegraf): void {
    ...
    bot.help(async (ctx) => {
      await ctx.reply(['/ask <текст>', '/rebuild_style', '/style'].join('\n'));
    });

    bot.command('ask', async (ctx) => {
      await this.handleIncomingText(ctx as unknown as TelegramTextContext, { forceReply: true });
    });
    ...
    bot.on('text', async (ctx) => {
      await this.handleIncomingText(ctx as unknown as TelegramTextContext, { forceReply: false });
    });
  ...
  private async handleIncomingText(
    ...
    if (!shouldReply || this.cooldown.isOnCooldown(ctx.chat.id, this.getNumber('BOT_REPLY_COOLDOWN_MS', 15000))) {
      return;
    }

    const userMessage = options.forceReply ? stripAskCommand(text) || text : text;

    await this.replyWithAi(ctx, userMessage);
  }

  private async replyWithAi(ctx: TelegramTextContext, userMessage: string): Promise<void> {
    try {
